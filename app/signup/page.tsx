    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 40px", background: "var(--bg)" }}>
        <div style={{ width: "100%", maxWidth: 370 }}>
          ...
          <Link href="/auth" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text-muted)", textDecoration: "none", marginBottom: 52, opacity: 0.8 }}>
            ← Back to log in
          </Link>
          ...
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: "0 0 36px", lineHeight: 1.5 }}>
            Start coaching in seconds — no credit card needed.
          </p>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} onFocus={() => setFocusedField("first")} onBlur={() => setFocusedField(null)} placeholder="Jane" required style={inputStyle("first")} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={labelStyle}>Last name</label>
                <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} onFocus={() => setFocusedField("last")} onBlur={() => setFocusedField(null)} placeholder="Smith" required style={inputStyle("last")} />
              </div>
            </div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", margin: 0 }}>
            ...
            <Link href="/auth" style={{ color: "var(--gold)", fontWeight: 700, textDecoration: "none" }}>Log in</Link>
          </p>
        </div>
      </div>
    </div>
